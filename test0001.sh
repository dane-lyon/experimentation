...
########################################################################
#suppression de l'applet network-manager (sauf si network précisé en 2ème argument)
########################################################################
if [ "$2" != "network" ] ; then 
...
if [ "$1" = "pi" ] ; then 
  ...
  chmod +x ubuntu-et-variantes-postinstall.sh ; ./ubuntu-et-variantes-postinstall.sh ; rm ubuntu-et-variantes-postinstall.sh ; clear
fi

if [ "$1" = "extra" ] ; then 
  wget --no-check-certificate https://raw.githubusercontent.com/simbd/Scripts_Ubuntu/master/Ubuntu18.04_Bionic_Postinstall.sh
  chmod +x Ubuntu18.04_Bionic_Postinstall.sh ; ./Ubuntu18.04_Bionic_Postinstall.sh ; rm Ubuntu18.04_Bionic_Postinstall.sh ; clear
fi
